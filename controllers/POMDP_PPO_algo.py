...
class PomdpPPO:
    def __init__(self, state_dim, action_dim, hidden_dim, recurrent_layers, max_action,
        ...
        self.replay_buffer = Episode(requires_distribution=True, requires_goal=False)
    ...
    def action(self, state, hidden_state):
        ...
        action_distribution, _, _, new_hidden_state = self.policy(state, hidden_state)
        action = action_distribution.rsample()
        return action.squeeze(0).data.cpu().numpy(), new_hidden_state

    def store(self, state, action, reward, next_state, done, others):
        self.replay_buffer.add(state, action, reward, next_state, done, others)

    def compute_advantage(self, gamma, lmbda, td_delta):
        td_delta = td_delta.detach().numpy().reshape(-1)
        advantage_list = []
        advantage = 0.0
    ...
    def update(self):
        ...
        hidden_states = (
            torch.stack([a_tuple[0] for a_tuple in sp["others"]], dim=1).squeeze(2),
            torch.stack([a_tuple[1] for a_tuple in sp["others"]], dim=1).squeeze(2),
        )

        td_target = rewards + self.gamma * self.critic(next_states, hidden_states) * (1 - dones)
        td_delta = td_target - self.critic(states, hidden_states)
        advantage = self.compute_advantage(self.gamma, self.lmbda, td_delta.cpu()).to(self.device)
        _, loc, var, _ = self.policy(states, hidden_states)
        old_action_dists = torch.distributions.MultivariateNormal(loc.detach(), var.detach())
        ...
        for i in range(self.epochs):
            ...
            left = self.critic(states, hidden_states)
